...
export function LogoIcon({ className = "", size = 20, spinOnHover = false }: LogoIconProps) {
  ...
  useEffect(() => {
    ...
    if (isHovered) {
      ...
      // Coast forward with momentum, then settle to upright (Rolls Royce style)
      const coast = 120;
      const currentHover = hoverRotate.get();
      const currentReverse = hoverRotateReverse.get();
      const targetHover = currentHover + coast;
      const targetReverse = currentReverse - coast;
      // Normalize to nearest upright (0 mod 360) for settle
      const normalizedHover = ((targetHover % 360) + 360) % 360;
      const normalizedReverse = ((targetReverse % 360) + 360) % 360;
      const settleHover = normalizedHover > 180 ? 360 : 0;
      const settleReverse = normalizedReverse > 180 ? 360 : 0;

      animate(hoverRotate, targetHover, {
        duration: 0.5,
        ease: [0.16, 1, 0.3, 1],
      }).then(() => {
        if (cancelled) return;
        animate(hoverRotate, settleHover, {
          type: "spring",
          stiffness: 65,
          damping: 16,
          mass: 1.4,
          onComplete: () => {
            if (!cancelled) hoverRotate.set(0);
          },
        });
      });
      animate(hoverRotateReverse, targetReverse, {
        duration: 0.5,
        ease: [0.16, 1, 0.3, 1],
      }).then(() => {
        if (cancelled) return;
        animate(hoverRotateReverse, settleReverse, {
          type: "spring",
          stiffness: 65,
          damping: 16,
          mass: 1.4,
          onComplete: () => {
            if (!cancelled) hoverRotateReverse.set(0);
          },
        });
      });
    }
