...
export function PolicyUploadStep({ onComplete }: PolicyUploadStepProps) {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto pt-8 pb-24 px-4 md:px-8">
      <div className="flex flex-col min-h-full w-full max-w-4xl mx-auto">
        {/* Welcome text — centered between top and three-step section */}
        <div className="flex-1 flex flex-col justify-center">
          ...
          >
                <h1
                  className="text-5xl md:text-5xl font-normal text-foreground-highlight"
                >
                  
                  <BrandName className="inline-flex items-center gap-1.5 text-4xl md:text-5xl font-normal align-baseline">
                  Meet{" "}<LogoIcon size={40} className="shrink-0 ml-1.5" />
                    Claire
                  </BrandName>
                </h1>
                <p className="text-muted max-w-md mx-auto -mt-12">
        ...
        {/* Three-bucket flow */}
        <div className="shrink-0 w-full mx-auto flex-1">
          ...
          {/* CTA placeholder when ready — keeps layout stable */}
          {phase === "ready" && <div className="mt-10 h-11" aria-hidden />}
        </div>
      </div>

      {/* Fixed CTA — always visible at viewport bottom when ready */}
      <AnimatePresence>
        {phase === "ready" && (
          <motion.div
            ...
            className="fixed bottom-6 sm:bottom-10 left-0 right-0 flex justify-center z-50 pointer-events-none"
          >
            <button
              type="button"
              onClick={onComplete}
